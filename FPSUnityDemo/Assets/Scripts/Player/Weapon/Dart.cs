...
public class Dart : MonoBehaviour
{
    ...
    void OnShoot()
    {
        ...
        if(true)
        {
            ...
            Vector3 cameraToMuzzle = (m_ProjectileBase.initialPosition - m_ProjectileBase.weaponCamera.transform.position);

            m_TrajectoryCorrectionVector = Vector3.ProjectOnPlane(-cameraToMuzzle, m_ProjectileBase.weaponCamera.transform.forward);
            ...
            if (Physics.Raycast(m_ProjectileBase.weaponCamera.transform.position, cameraToMuzzle.normalized, out RaycastHit hit, cameraToMuzzle.magnitude, hittableLayers, k_TriggerInteraction))
            {
                if (IsHitValid(hit))
                {
    ...
    //     Gizmos.DrawRay(transform.position, correctionThisFrame);
    // }
